...
def main():
    ...
    if unet.conv_in.weight.shape[1] != 8:
        with torch.no_grad():
            ...
            #print(new_conv_in.weight.shape, unet.conv_in.weight.shape)
    ...
    def collate_fn(examples):
        ...
        original_pixel_values = torch.stack([example["original_pixel_values"][0] for example in examples])
        original_pixel_values = original_pixel_values.to(memory_format=torch.contiguous_format).float()
        # edited_pixel_values = torch.stack([example["edited_pixel_values"] for example in examples])
        edited_pixel_values = torch.stack([example["edited_pixel_values"][0] for example in examples])
    ...
    if accelerator.is_main_process:
        # accelerator.init_trackers("instruct-pix2pix", config=vars(args))if accelerator.is_main_process:
            ...
            if accelerator.is_main_process:
                ...
                if args.validation_prompt is not None \
                    ...
                    for validation_prompt, validation_image in (zip(validation_prompts, validation_images)):
                        ...
                        original_image = validation_image.resize((args.resolution, args.resolution))####
                        # run inference
                        # original_image = download_image(args.val_image_url)
                        edited_images = [original_image]
                        with torch.autocast(
                            ...
                            for _ in range(args.num_validation_images):
                                ...
                                # undeltad = undeltad.clip(0,255).astype(np.uint8)
                                # edited_images[-1] = PIL.Image.fromarray(undeltad)
                                # #######

                        for tracker in accelerator.trackers:
                            if tracker.name == "tensorboard":
                                np_images = np.stack([np.asarray(img) for img in edited_images])
                                tracker.writer.add_images("validation/"+validation_prompt, np_images, global_step, dataformats="NHWC")
                            
                            if tracker.name == "wandb":
                                wandb_table = wandb.Table(columns=WANDB_TABLE_COL_NAMES)
                                for edited_image in edited_images:
                                    wandb_table.add_data(
                                        wandb.Image(original_image), wandb.Image(edited_image), validation_prompt
                                    )
