...
def loss_fcn(painting, target, use_clip_loss=False):
    loss = 0 
    ...
    if use_clip_loss:
        loss += clip_conv_loss(painting, target) * 0.25
    return loss
...
def plan_all_strokes(opt, optim_iter=100, num_strokes=120, num_passes=3):
    ...
    for i in range(num_passes):
        ...
        optim = torch.optim.Adam(painting.parameters(), lr=5e-3 * (len(painting.brush_strokes)/100))
        for j in tqdm(range(optim_iter)):
            optim.zero_grad()
            p = painting(strokes=strokes_small)
            loss = 0
            loss += loss_fcn(p, target,  use_clip_loss=False)
            loss.backward()
