...
def process_stroke_library(raw_strk_lib):
    ...
    for x_start in np.linspace(0,CANVAS_WIDTH, num=cells_x+1)[:-1]:
        for y_start in np.linspace(0, CANVAS_HEIGHT, num=cells_y+1)[:-1]:
            ...
            #canvas = apply_stroke(canvas, stroke, color, x, y)
            #canvas = apply_stroke(canvas, stroke, color, x_start_pix, y_start_pix)
            canvas = apply_stroke(canvas, stroke, len(strokes), color, 
