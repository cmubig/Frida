...
class PaintingExecutor():
    ...
    def compute_plann(self):
        pass

    def execute_plann(self):
        pass
...
if __name__ == '__main__':
    ...
    if opt.webcam_interface:
        print('Starting webcam interface')
        #set --objective_data to the path of the webcam portrait
        webcam_interface = WebcamInterface()
        print(f'last portrait: {webcam_interface.last_portrait_path}')
        opt.objective_data = [webcam_interface.last_portrait_path]
        # print(type(opt.objective_data), opt.objective_data)
        error = webcam_interface.run()
        # if error:
        #     print('exiting')
        #     sys.exit(1)
    ...
    if opt.recover_painting:
        print(" ------- Recovering painting from last run -------")
        #load painting
        with open(opt.painting_path + '/painting.pkl', 'rb') as f:
            painting = pickle.load(f)
        with open(opt.painting_path + '/color_palette.pkl', 'rb') as f:
    ...
    print(painting)
    if opt.save_painting:
        # create directory to save painting
        if not os.path.exists(opt.painting_path):
            os.makedirs(opt.painting_path)

        #save painting as pickle object
        with open(opt.painting_path + '/painting.pkl', 'wb') as f:
            pickle.dump(painting, f)
        
        # save color_palette
        with open(opt.painting_path + '/color_palette.pkl', 'wb') as f:
