...
def plan_from_image(opt):
    ...
    painting = initialize_painting(opt, opt.num_strokes, target_img, current_canvas, opt.ink)
    ...
    for i in (range(0, opt.num_strokes, stroke_batch_size)):#, desc="Initializing"):
        painting = add_strokes_to_painting(opt, painting, painting(h,w)[:,:3], stroke_batch_size, 
...
def plan(opt):
    ...
    if opt.use_colors_from is None and not opt.ink:
        colors = painting.cluster_colors(opt.n_colors)
...
def adapt(opt):
    ...
    painting = Painting(opt, 0, background_img=current_canvas, brush_strokes=brush_strokes).to(device)
...
def load_objectives_data(opt):
    ...
    for i in range(len(opt.init_objective) if opt.init_objective else 0):
        if opt.init_objective[i] == 'text':
            ...
            with torch.no_grad():
                img = load_img(opt.init_objective_data[i],h=opt.h_render, w=opt.w_render).to(device)/255.
...
def optimize_painting(opt, painting, optim_iter, color_palette=None,
    ...
    for it in tqdm(range(optim_iter), desc='Optimizing {} Strokes'.format(str(len(painting.brush_strokes)))):
        ...
        for k in range(len(opt.objective)):
            loss += parse_objective(opt.objective[k], 
                opt.objective_data_loaded[k], p[:,:3], weight=opt.objective_weight[k])
        #loss += (1-alphas).mean() * opt.fill_weight
        loss += torch.abs(1-alphas).mean() * opt.fill_weight
        loss.backward()
