...
class StableDiffusion(nn.Module):
    ...
    def train_step(self, text_embeddings, pred_rgb, guidance_scale=100):
        ...
        pred_rgb_512 = F.interpolate(pred_rgb, (128, 128), mode='bilinear', align_corners=False)
        ...
        return grad#0 # dummy loss value
    ...
    def encode_imgs(self, imgs):
        ...
        # print(vars(posterior))

        posterior = self.vae.encode(imgs).latent_dist
        latents = posterior.sample() * 0.18215

        # def sample(posterior, generator=None) -> torch.FloatTensor:
        #     device = 'cuda'
        #     sample_device = 'cuda'
        #     sample = torch.randn(posterior.mean.shape, generator=generator, device=sample_device)
        #     # make sure sample is on the same device as the parameters and has same dtype
        #     sample = sample.to(device=device, dtype=posterior.parameters.dtype)
        #     x = posterior.mean + posterior.std * sample
        #     return x
        # posterior = self.vae.encode(imgs)
        # latents = sample(posterior) * 0.18215
