...
class StrokePredictor(nn.Module):
    def __init__(self, opt,
        ...
        if opt.sp_predict_path:
            self.stroke_latent_size = 32*3
    ...
    def forward(self, current_canvas, target_canvas, n_strokes=None, training=True, init_differentiably=True):
        '''
        ...
        for batch_ind in range(len(current_canvas)):
            ...
            for stroke_ind in range(n_strokes):
                latent = latents[batch_ind, stroke_ind:stroke_ind+1, :]#.squeeze()
                a =     rotation[batch_ind, stroke_ind, :]
                xt =    position[batch_ind, stroke_ind, :1]
                yt =    position[batch_ind, stroke_ind, -1:]
                if self.opt.sp_predict_path:
                    path = latent.view(-1,32,3)
                    bs = BrushStroke(self.opt, init_differentiably=init_differentiably,
                                    ink=True,  path=path, a=a,
                                    xt=xt, yt=yt)
                else:
                    bs = BrushStroke(self.opt, init_differentiably=init_differentiably,
...
def brush_stroke_parameter_loss_fcn(predicted_strokes, true_strokes, param2img):
    ...
    loss_x, loss_y, loss_rot, loss_latent, loss_wasserstein, loss_path = 0,0,0,0,0, 0
    ...
    for batch_ind in range(len(predicted_strokes)):
        ...
        for stroke_ind in range(len(predicted_strokes[batch_ind])):
            ...
            if (pred_bs.latent is not None) and (true_bs.latent is not None):
                loss_latent += l1_loss(pred_bs.latent, true_bs.latent) #* 1e-3 ############################
                # print(pred_bs.latent.shape, true_bs.latent.shape)
            if (pred_bs.path is not None):
                with torch.no_grad():
                    true_path = true_bs.get_path()
                loss_path += l1_loss(pred_bs.path, true_path)
    ...
    n = n_batch * n_strokes
    loss_x, loss_y, loss_rot, loss_latent, loss_path, loss_wasserstein = loss_x/n, loss_y/n, loss_rot/n, loss_latent/n, loss_path/n, loss_wasserstein/n

    loss = loss_x + loss_y + loss_rot + loss_latent + loss_path #+ loss_wasserstein
    # loss = loss_path

    return loss, loss_x, loss_y, loss_rot, loss_latent, loss_path, loss_wasserstein
